...
function renderSessions() {
  ...
  all = typeof _filterSessionsForUser === 'function' ? _filterSessionsForUser(all) : all;
...
function viewSession(id) {
  ...
  // Kabul durumu badge
  const fsBg = s.finalStatus==='ok' ? '#d1fae5' : s.finalStatus==='nok' ? '#fee2e2' : '#f1f5f9';
  const fsC  = s.finalStatus==='ok' ? '#065f46' : s.finalStatus==='nok' ? '#991b1b' : '#64748b';
  const fsT  = s.finalStatus==='ok' ? '✅ OK — Kabul' : s.finalStatus==='nok' ? '❌ NOK — Red' : '—';
  const metaHtml =
...
function delSession(id) {
  ...
  renderSessions(); if(typeof renderDash==='function') renderDash();
...
function downloadPDF(id) {
  ...
  if (s.formType === 'measurement') {
    ...
    bodyHtml =
      ...
      '<div style="display:flex;gap:4px;font-size:11px"><span style="font-weight:700;min-width:82px;color:#374151">Durum:</span>' +
      '<span style="background:'+stBg+';color:'+stC+';padding:1px 7px;border-radius:4px;font-weight:700;font-size:10px">'+stLbl+'</span></div>' +
      (s.finalStatus ? (() => { const fb=s.finalStatus==='ok'?'#d1fae5':'#fee2e2',fc=s.finalStatus==='ok'?'#065f46':'#991b1b',ft=s.finalStatus==='ok'?'✅ OK — KABUL':'❌ NOK — RED'; return '<div style="display:flex;gap:4px;font-size:11px"><span style="font-weight:700;min-width:82px;color:#374151">Kabul:</span><span style="background:'+fb+';color:'+fc+';padding:1px 7px;border-radius:4px;font-weight:700;font-size:10px">'+ft+'</span></div>'; })() : '') +
      (s.note ? '<div style="grid-column:1/-1">'+mr('Not',s.note)+'</div>' : '') +
      (s.formNote ? '<div style="grid-column:1/-1">'+mr('Genel Not',s.formNote)+'</div>' : '') +
      '</div>' +
