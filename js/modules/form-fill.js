...
function startFill() {
  ...
  _fillD = {
    ...
    condNote:    _editSid ? (D.sessions.find(s=>s.id===_editSid)||{}).condNote||''    : '',
...
function _buildFillUI() {
  ...
  const condSty = prevFS==='cond' ? 'background:#e0f2fe;border-color:#0284c7;color:#0c4a6e;' : '';
  ...
  html += '<div class="fill-sec" style="border:2px solid var(--border);border-radius:10px;padding:14px;background:#f8fafc;margin-top:4px">' +
    ...
    '<div style="display:flex;gap:8px;margin-bottom:14px;flex-wrap:wrap">' +
      '<label id="fs-ok-lbl" style="display:flex;align-items:center;gap:7px;padding:9px 14px;border:2px solid var(--border);border-radius:9px;cursor:pointer;flex:1;min-width:120px;font-size:.85rem;font-weight:700;transition:all .15s;'+okSty+'">' +
      ...
      '</label>' +
      '<label id="fs-cond-lbl" style="display:flex;align-items:center;gap:7px;padding:9px 14px;border:2px solid var(--border);border-radius:9px;cursor:pointer;flex:1;min-width:140px;font-size:.85rem;font-weight:700;transition:all .15s;'+condSty+'">' +
        '<input type="radio" name="final-status" value="cond" '+(prevFS==='cond'?'checked':'')+' onchange="_onFinalStatus()" style="accent-color:#0284c7;width:16px;height:16px"> 🔵 Şartlı Kabul' +
      '</label>' +
      '<label id="fs-nok-lbl" style="display:flex;align-items:center;gap:7px;padding:9px 14px;border:2px solid var(--border);border-radius:9px;cursor:pointer;flex:1;min-width:120px;font-size:.85rem;font-weight:700;transition:all .15s;'+nokSty+'">' +
      ...
      '</label>' +
    '</div>' +
    '<div id="fs-cond-note-wrap" style="display:'+(prevFS==='cond'?'':'none')+';margin-bottom:12px">' +
      '<div class="fg" style="margin-bottom:0">' +
        '<label style="color:#0284c7">🔵 Şartlı Kabul Koşulu / Gereksinim *</label>' +
        '<input type="text" id="fs-cond-note" placeholder="Örn: 3 gün içinde tamir edilecek, ilk kontrolde onaylanacak..." value="'+esc((_fillD.condNote)||'')+'" style="border-color:#0284c7">' +
      '</div>' +
    '</div>' +
    '<div class="fg" style="margin-bottom:0">' +
...
function _onFinalStatus() {
  ...
  const condLbl = document.getElementById('fs-cond-lbl');
  const nokLbl  = document.getElementById('fs-nok-lbl');
  const condWrap = document.getElementById('fs-cond-note-wrap');
  ...
  if (condLbl) {
    condLbl.style.background  = val==='cond' ? '#e0f2fe' : '';
    condLbl.style.borderColor = val==='cond' ? '#0284c7' : 'var(--border)';
    condLbl.style.color       = val==='cond' ? '#0c4a6e' : '';
  }
  if (nokLbl) {
  ...
  if (condWrap) condWrap.style.display = val==='cond' ? '' : 'none';
}
...
function _collect() {
  ...
  // Şartlı kabul koşulu zorunlu
  const condNoteEl = document.getElementById('fs-cond-note');
  _fillD.condNote = condNoteEl ? condNoteEl.value.trim() : '';
  if (_fillD.finalStatus === 'cond' && !_fillD.condNote) { valid = false; }
...
function saveAsFinal() {
  ...
  if (_fillD && _fillD.formType === 'measurement') {
    ...
    if (!ok)  { toast('⚠️ Uygun Değil / Sapmalı maddeler için sebep girin!\nŞartlı Kabul seçildiyse koşul alanı zorunludur.','err',4000); return; }
  }
